...
>
    ...
    <td>
        <x-text-input name="class_mark[]" class="min-w-24 md:min-w-20 {{ $painttd }}" x-model="c_score" @input="total=parseInt(c_score)+parseInt(e_score)" :readonly="$readonly" />
        <x-input-error :messages="$errors->get('class_mark.'.$key)" class="mt-2" />
    </td>
    <td>
        <x-text-input name="exam_mark[]" class="min-w-24 md:min-w-20 {{ $painttd }}" x-model="e_score" @input="total=parseInt(c_score)+parseInt(e_score)" :readonly="$readonly" />
