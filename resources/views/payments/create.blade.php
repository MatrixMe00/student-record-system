<x-app-layout>
    ...
    <x-app-main>
        @if (session('payment_is_ready'))
            ...
            @else
                <x-form-container maintitle="Make Payment for Service" subtitle="Use the form below to make payment for the service requested" class="bg-zinc-100">
                    <x-form-element action="" method="POST" id="paymentForm">
                        @if ($type != "results")
                            <x-info-card class="text-sm lg:col-span-2 text-center w-fit m-auto border" shadow="">{!! __("An additional 5% has been added to the original price <b>GH¢ ".number_format($original,2)."</b>. This is used to cover all electronic charges") !!}</x-info-card>
                        @endif
                        {{-- contact name --}}
                        <div>
            ...
            @endif
        @else
            <x-empty-div>{{ __("You cannot make payment yet. Contact school admin for further assistance.") }}</x-empty-div>
        @endif
    </x-app-main>
</x-app-layout>
