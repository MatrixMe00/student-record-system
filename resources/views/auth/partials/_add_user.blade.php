<x-form-container maintitle="Add A New User" subtitle="Fill the form below to add a new user" x-data="{ selected_role:'{{ old('role_id') }}', email_val:'{{ old('email') }}', index_number:'{{ $index_number }}', username:'{{ old('username') }}' }" x-init="email_val='{{ old('email') }}'">
    <x-form-element action="{{ route('register') }}" method="POST">
        {{-- user role --}}
        <div>
        ...
        <div x-show="selected_role != 5">
            ...
            <x-text-input id="password" type="password" name="password" :value="old('password')" placeholder="Password" />
        ...
        <div>
            ...
            <x-text-input id="primary_phone" type="tel" name="primary_phone" :value="old('primary_phone')" placeholder="XXX XXX XXXX" maxlength="13" minlength="10" />
        ...
        <x-form-button-container>
            <x-form-submit-button icon="fas fa-user-plus">Create User Account</x-form-submit-button>
        </x-form-button-container>
    </x-form-element>
</x-form-container>
